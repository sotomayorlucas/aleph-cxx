...
- [ ] **Step 3 — Metal pinned-vertex specular** (eye `{0,0,5}`): pick a front-facing `on_sphere`-style vertex normal `N0`; `Ldir = N0*(2*dot(V,N0)) − V`; place a light entity centered at `N0_point + Ldir*4`, emit `{3,3,3}`. Build with `fuzz=0` then `fuzz=1`; assert that vertex's lum(fuzz0) > lum(fuzz1) and > its no-light env lum. (Verify `dot(N0, normalize(normalize(center−N0_point)+V)) ≈ 1` in the test as a guard.)
